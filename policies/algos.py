...
def sac_train(
    ...
    optimizers = [policy.actor.optimizer, critic.optimizer, ent_coef_optimizer]
    ...
    for _ in range(gradient_steps):
        ...
        current_q_values = critic(replay_data.observations, replay_data.actions.float())
        # if policy_name == 'intervene':
        #     current_q_values = critic(replay_data.observations, replay_data.actions.float())
        # else:
        #     current_q_values = critic(replay_data.observations, replay_data.actions.float(), other_replay_data.actions.float())
        ...
        q_values_pi = torch.cat(critic.forward(replay_data.observations, actions_pi), dim=1)
        # if policy_name == 'intervene':
        ...
        #     q_values_pi = torch.cat(critic.forward(replay_data.observations, actions_pi, other_actions), dim=1)
...
# Assumes policy is the intervention policy
def two_action_sac_train(
    policy,
    replay_buffer: ReplayBuffer, 
    log_ent_coef,
    ent_coef_optimizer,
    ...
    episode:int = None,
    policy_name:str = None,
    task_policy: DoubleCriticSACPolicy = None,
    intervene: bool = False
) -> None:
    """Copied from SAC in stable baselines"""
    # Update optimizers learning rate
    ...
    for _ in range(gradient_steps):
        ...
        with torch.no_grad():
            ...
            for _ in range(n_samples):
                next_other_actions = task_policy.forward(replay_data.next_observations, deterministic=False)
                next_q_values += torch.cat(policy.critic_target(replay_data.next_observations, next_actions, next_other_actions), dim=1)
            next_q_values /= n_samples
        ...
        other_actions_list = []
        for _ in range(n_samples):
            other_actions_list.append(task_policy.forward(replay_data.next_observations, deterministic=False))
        current_q_values = None
        for other_actions in other_actions_list:
            critic_out = critic(replay_data.observations, replay_data.actions.float(), other_actions)
            if current_q_values is None:
                current_q_values = list(critic_out)
            else:
                for i in range(len(current_q_values)):
                    current_q_values[i] += critic_out[i]
                    current_q_values[i] /= n_samples
        current_q_values = list(current_q_values)
...
# Assumes policy is the task policy
def double_critic_sac_train(
    policy: DoubleCriticSACPolicy,
    replay_buffer: ReplayBuffer, 
    log_ent_coef,
    ent_coef_optimizer,
    lr_schedule, 
    ...
    for _ in range(gradient_steps):
        ...
        with torch.no_grad():
            ...
            # next_q_values = 0
            # for _ in range(n_samples):
            #     next_interventions, next_intervene_log_prob = intervene_policy.actor.action_log_prob(replay_data.next_observations)
            #     next_q_values += torch.cat(policy.critic_target(replay_data.next_observations, next_interventions, next_actions), dim=1)
            # next_q_values /= n_samples
            next_interventions, next_intervene_log_prob = intervene_policy.actor.action_log_prob(replay_data.next_observations)
            ...
            next_q_values, _ = torch.min(next_q_values, dim=1, keepdim=True)
            # add entropy term
            next_q_values = next_q_values - ent_coef * next_log_prob.reshape(-1, 1) * next_intervene_log_prob.reshape(-1, 1)
            # td error + entropy term
            target_q_values = replay_data.rewards + (1 - replay_data.dones) * gamma * next_q_values
        ...
        # intervenes_list = []
        # for _ in range(n_samples):
        #     intervenes_list.append(intervene_policy.forward(replay_data.next_observations, deterministic=False))
        # current_q_values = None
        # for intervenes in intervenes_list:
        #     critic_out = critic(replay_data.observations, intervenes, replay_data.actions.float())
        #     if current_q_values is None:
        #         current_q_values = list(critic_out)
        #     else:
        #         for i in range(len(current_q_values)):
        ...
        #             current_q_values[i] /= n_samples
        # current_q_values = list(current_q_values)
        current_q_values = critic(replay_data.observations, replay_data.intervenes, replay_data.actions.float())
        ...
        # q_values_pi = 0
        # for intervenes in intervenes_list:
        #     q_values_pi += torch.cat(critic.forward(replay_data.observations, intervenes, actions_pi), dim=1)
        # q_values_pi /= n_samples
        q_values_pi = torch.cat(critic.forward(replay_data.observations, replay_data.intervenes, actions_pi), dim=1)
        min_qf_pi, _ = torch.min(q_values_pi, dim=1, keepdim=True)
        actor_loss = (ent_coef * log_prob - min_qf_pi).mean()
        actor_losses.append(actor_loss.item())
