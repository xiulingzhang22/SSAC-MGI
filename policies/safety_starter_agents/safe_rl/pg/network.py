...
from .utils import combined_shape, EPS
...
def mlp_actor_critic(x, a, hidden_sizes=(64,64), activation=tf.tanh,
                     output_activation=None, policy=None, action_space=None, agent_type=None):
    ...
    if agent_type is not None:
        pi_scope = f'pi_{agent_type}'
        vf_scope = f'vf_{agent_type}'
        vc_scope = f'vc_{agent_type}'
    else:
        pi_scope = 'pi'
        vf_scope = 'vf'
        vc_scope = 'vc'
    with tf.variable_scope(pi_scope):
    ...
    with tf.variable_scope(vf_scope):
        v = tf.squeeze(mlp(x, list(hidden_sizes)+[1], activation, None), axis=1)

    with tf.variable_scope(vc_scope):
