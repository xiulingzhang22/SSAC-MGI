...
from ..utils.mpi_tools import mpi_fork, mpi_sum, proc_id, mpi_statistics_scalar, num_procs
...
def evaluate(env, get_action, episodes: int, cumulative_cost: float):
    ...
    return np.mean(ep_returns), np.mean(ep_costs), np.mean(ep_correct_goals_arr)

"""
Soft Actor-Critic
"""
def sac(env_fn, results_path, actor_fn=mlp_actor, critic_fn=mlp_critic, ac_kwargs=dict(), seed=0,
        steps_per_epoch=1000, replay_size=int(1e6), gamma=0.99,
        ...
        reward_scale=1, min_episodes=2000, algorithm='sac', gradient_steps=20
        ):
    """
    ...
    for t in range(total_steps // num_procs()):
        ...
        if all_steps % 1000 == 0:
            ...
            f.write(f'{all_steps}, {mean_eval_return}, {mean_eval_cost}, {cumulative_cost}, {mean_correct_goals}\n')
            f.flush()
        ...
        if d or (ep_len == max_ep_len):
            ...
            print(f'Algorithm: {algorithm}, Episode: {episode}, Reward: {np.mean(all_rewards[-100:])}, Cost: {np.mean(all_costs[-100:])}, Steps: {steps}')
            o, r, d, ep_ret, ep_cost, ep_len, ep_goals = env.reset(), 0, False, 0, 0, 0, 0
