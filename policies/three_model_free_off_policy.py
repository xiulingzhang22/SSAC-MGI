...
class ThreeModelFreeOffPolicy(Policy):
    ...
    def get_action(self, 
        ...
        observation = torch.as_tensor(np.array([observation])).to(self.device)
        standard_action = \
        ...
        #Mapping the [-1,1] output of the intervention network to [0,1]
        #
        # picked action is <1.2 else we don't intervene.

        intervene = True if intervene_action >= 0.5 else False #0.5 0.7
