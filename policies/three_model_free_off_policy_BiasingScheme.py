...
class ThreeModelFreeOffPolicy(Policy):
    ...
    def __init__(
        ...
        # Here, the value of power can be modified to adjust the intervention degree.
        # When power is greater than 1, the intervention policy begins to favor intervention
        self.intervene_action_space = gym.spaces.Box(0, 1, shape=(1,))
        self.action_range = [self.intervene_action_space.low[0], self.intervene_action_space.high[0]]
        self.power = 0.6
    ...
    # Then f(x) = x^n | n ∈ (0, 2),
    def mapping_bias(self, action, action_space_low, action_space_high, bias):
        mapped_action = action_space_low + \
                        ((action_space_high - action_space_low) / 2) * (action + 1)
        biased_action = mapped_action ** bias
    ...
    def get_action(self, 
        ...
        # Range of intervene_action from[-1,1] mapping to[0,1]
        # If the value obtained by exponential conversion of mapped intervention action is less than 0.5,
        # intervention will be carried out; otherwise, no intervention will be conducted
        # print("self.action_range[0],self.action_range[1],self.power",self.action_range[0],self.action_range[1],self.power)
        intervene_action = self.mapping_bias(intervene_action, self.action_range[0], self.action_range[1], self.power)
        intervene = True if intervene_action < 0.5 else False
        #Original:
