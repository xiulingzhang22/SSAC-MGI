...
from stable_baselines3.common.type_aliases import (
    ...
    ReplayBufferSamples,
    InterventionReplayBufferSamples,
...
class InterventionReplayBuffer(BaseBuffer):
    """
    Replay buffer used in off-policy algorithms like SAC/TD3.
    ...
    def __init__(
        ...
        super(InterventionReplayBuffer, self).__init__(buffer_size, observation_space, action_space, device, n_envs=n_envs)
        ...
        self.intervenes = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
        ...
        if psutil is not None:
            total_memory_usage = self.observations.nbytes + self.intervenes.nbytes + self.actions.nbytes + self.rewards.nbytes + self.dones.nbytes
    ...
    def add(
        ...
        self.intervenes[self.pos] = np.array(intervene).copy()
        self.actions[self.pos] = np.array(action).copy()
        self.rewards[self.pos] = np.array(reward).copy()
        self.dones[self.pos] = np.array(done).copy()
    ...
    def _get_samples(self, batch_inds: np.ndarray, env: Optional[VecNormalize] = None) -> ReplayBufferSamples:
        ...
        data = (
            ...
            self.actions[batch_inds, 0, :],
            next_obs,
            self.intervenes[batch_inds],
            # Only use dones that are not due to timeouts
            # deactivated by default (timeouts is initialized as an array of False)
            self.dones[batch_inds] * (1 - self.timeouts[batch_inds]),
            self._normalize_reward(self.rewards[batch_inds], env),
        )
        return InterventionReplayBufferSamples(*tuple(map(self.to_torch, data)))
