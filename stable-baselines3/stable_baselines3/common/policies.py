...
class ContinuousCritic(BaseModel):
    ...
    def __init__(
        ...
        n_critics: int = 2,
        share_features_extractor: bool = True,
        use_other_player_actions: bool = False,
        action_dim2: int = 0
    ):
        super().__init__(
            observation_space,
        ...
        action_dim1 = get_action_dim(self.action_space)

        self.share_features_extractor = share_features_extractor
        self.n_critics = n_critics
        self.q_networks = []
        for idx in range(n_critics):
            if self.use_other_player_actions:
                q_net = create_mlp(features_dim + action_dim1 + action_dim2, 1, net_arch, activation_fn)
            else:
                q_net = create_mlp(features_dim + action_dim1 + action_dim2, 1, net_arch, activation_fn)
            q_net = nn.Sequential(*q_net)
            self.add_module(f"qf{idx}", q_net)
    ...
    def forward(self, obs: th.Tensor, actions: th.Tensor, actions2: th.Tensor = None) -> Tuple[th.Tensor, ...]:
        # Learn the features extractor using the policy loss only
        # when the features_extractor is shared with the actor
        with th.set_grad_enabled(not self.share_features_extractor):
            features = self.extract_features(obs)
        if self.use_other_player_actions:
            qvalue_input = th.cat([features, actions, actions2], dim=1)
        else:
            qvalue_input = th.cat([features, actions], dim=1)
        return tuple(q_net(qvalue_input) for q_net in self.q_networks)

    def q1_forward(self, obs: th.Tensor, actions: th.Tensor, actions2: th.Tensor = None) -> th.Tensor:
        """
        Only predict the Q-value using the first network.
        This allows to reduce computation when all the estimates are not needed
        (e.g. when updating the policy in TD3).
        ...
        with th.no_grad():
            features = self.extract_features(obs)
        if self.use_other_player_actions:
            return self.q_networks[0](th.cat([features, actions, actions2], dim=1))
        else:
            return self.q_networks[0](th.cat([features, actions], dim=1))


class DoubleContinuousCritic(BaseModel):
    """
    Critic network(s) for DDPG/SAC/TD3.
    It represents the action-state value function (Q-value function).
    Compared to A2C/PPO critics, this one represents the Q-value
    ...
    def __init__(
        ...
        self.share_features_extractor = share_features_extractor
        self.n_critics = n_critics
        self.standard_q_networks = []
        self.safe_q_networks = []
        for idx in range(n_critics):
            if self.use_other_player_actions:
                standard_q_net = create_mlp(features_dim + 2*action_dim, 1, net_arch, activation_fn)
                safe_q_net = create_mlp(features_dim + 2*action_dim, 1, net_arch, activation_fn)
            else:
                standard_q_net = create_mlp(features_dim + action_dim, 1, net_arch, activation_fn)
                safe_q_net = create_mlp(features_dim + action_dim, 1, net_arch, activation_fn)
            standard_q_net = nn.Sequential(*standard_q_net)
            safe_q_net = nn.Sequential(*safe_q_net)
            self.add_module(f"standard_qf{idx}", standard_q_net)
            self.add_module(f"safe_qf{idx}", safe_q_net)
            self.standard_q_networks.append(standard_q_net)
            self.safe_q_networks.append(safe_q_net)
    ...
    def forward(
        ...
        if self.use_other_player_actions:
            ...
            qvalue_input = th.cat([features, actions], dim=1)
        return self._get_q_vals(qvalue_input, interventions)

    def q1_forward(self, obs: th.Tensor, interventions: th.Tensor, actions: th.Tensor, actions2: th.Tensor = None) -> th.Tensor:
        """
        Only predict the Q-value using the first network.
        This allows to reduce computation when all the estimates are not needed
        (e.g. when updating the policy in TD3).
        ...
        with th.no_grad():
            features = self.extract_features(obs)
        if self.use_other_player_actions:
            return self._get_q_val_batch(th.cat([features, actions, actions2], dim=1), interventions, 0)
        else:
            return self._get_q_val_batch(th.cat([features, actions], dim=1), interventions, 0)
        
    def _get_q_val_batch(self, q_val_inputs, interventions, critic_idx):
        standard_q_net = self.standard_q_networks[critic_idx]
        safe_q_net = self.safe_q_networks[critic_idx]
        standard_q_vals = standard_q_net(q_val_inputs)
        safe_q_vals = safe_q_net(q_val_inputs)
        return standard_q_vals * (1. - interventions) \
            + safe_q_vals * interventions
        
    def _get_q_vals(self, q_val_inputs, interventions):
        q_vals = []
        for critic_idx in range(self.n_critics):
            q_val_batch = self._get_q_val_batch(q_val_inputs, interventions, critic_idx)
            q_vals.append(q_val_batch)
        return tuple(q_vals)
